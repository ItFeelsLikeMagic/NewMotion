...
| Default bundle prefix | `com.davidliao.newmotion` (in `project.yml` and the fallback generator) |
...
`com.davidliao.newmotion` is now the default everywhere: the fallback
...
loop. `NEWMOTION_BUNDLE_PREFIX` still overrides the whole prefix for a
different signing account, and `NEWMOTION_IOS_BUNDLE_ID` still overrides the
...
./scripts/generate.sh
./scripts/build.sh
NEWMOTION_RUN_IOS_TESTS=1 ./scripts/test.sh
./scripts/fuzz-protocol.sh
```

`test.sh` picks the first available iPhone simulator. Set
`NEWMOTION_IOS_DESTINATION='platform=iOS Simulator,id=<uuid>'` to choose one.
...
run. `NEWMOTION_INERT_HOST=1` forces the same behaviour outside tests.

The two tests that use the real Keychain are opt-in:

```bash
NEWMOTION_KEYCHAIN_TESTS=1 ./scripts/test.sh
```
...
export NEWMOTION_SIGNING=1
export NEWMOTION_DEVELOPMENT_TEAM=<your team id>
export NEWMOTION_DERIVED_DATA=/tmp/NewMotionPhoneInstall
export NEWMOTION_BUNDLE_PREFIX=com.davidliao.newmotion
...
```bash
export IPHONE_UDID=<from devicectl list devices>
export NEWMOTION_IOS_BUNDLE_ID=com.davidliao.newmotion.ios
./scripts/launch-phone.sh
./scripts/debug-phone.sh
```

`debug-phone.sh` finds the phone by name (`dliao's iPhone`, override
`NEWMOTION_DEVICE_NAME`) and copies to `/tmp/newmotion-phone-debug`.
...
NEWMOTION_LOG_WINDOW=5m ./scripts/logs-mac.sh
```

`install-mac.sh` replaces and launches `~/Applications/NewMotion.app`. Grant
...
boost list. `NEWMOTION_DEBUG_SERVER=0` disables the Mac debug server.
...
leaves `NewMotion.app` and `NewMotion.zip` in `build/dist`. The zip
...
- `NEWMOTION_DEVELOPMENT_TEAM`, the ten-character team id.
- `NEWMOTION_NOTARY_PROFILE`, the name of a notarytool keychain profile.
