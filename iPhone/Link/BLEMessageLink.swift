...
final class BLEMessageLink: MessageLink, @unchecked Sendable {
    ...
    private func deliver(_ message: Data, on channel: LinkChannel, delivery: LinkDelivery) -> LinkSendResult {
        ...
        // left.  Either way it goes only if the whole of it fits, because half
        // a message is one the Mac can never put back together, and a caller
        // told `busy` after the first fragment has already lost the rest.
        let queued = delivery.mayQueue
        if fragments.count > peripheral.queueCapacity(on: channel) { return .busy }
